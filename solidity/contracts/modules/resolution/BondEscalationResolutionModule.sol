...
contract BondEscalationResolutionModule is Module, IBondEscalationResolutionModule {
  ...
  function startResolution(
    ...
    IOracle.Dispute calldata
  ) external onlyOracle {
    escalations[_disputeId].startTime = uint128(block.timestamp);
    emit ResolutionStarted(_response.requestId, _disputeId);
  }
  ...
  function pledgeForDispute(
    ...
    _pledge(_request, _response, _dispute, _disputeId, _pledgeAmount, true);
  }
  ...
  function pledgeAgainstDispute(
    ...
    _pledge(_request, _response, _dispute, _disputeId, _pledgeAmount, false);
  }
  ...
  function resolveDispute(
    ...
    emit DisputeResolved(_response.requestId, _disputeId, _disputeStatus);
  }

  /// @inheritdoc IBondEscalationResolutionModule
  function claimPledge(IOracle.Request calldata _request, bytes32 _requestId, bytes32 _disputeId) external {
    Escalation storage _escalation = escalations[_disputeId];
    ...
    if (_escalation.resolution == Resolution.DisputerWon) {
      ...
      _claimPledge(_request.resolutionModuleData, _requestId, _disputeId, _amountToRelease, _escalation.resolution);
      ...
      if (_pledgerBalanceFor > 0) {
        ...
        _claimPledge(_request.resolutionModuleData, _requestId, _disputeId, _pledgerBalanceFor, _escalation.resolution);
      }

      if (_pledgerBalanceAgainst > 0) {
        pledgesAgainstDispute[_disputeId][msg.sender] -= _pledgerBalanceAgainst;
        _claimPledge(
          _request.resolutionModuleData, _requestId, _disputeId, _pledgerBalanceAgainst, _escalation.resolution
        );
      }
    }
  }
   ...
   *
   * @param _request The ID of the request
   * @param _response The ID of the dispute
  ...
  function _pledge(
    ...
    // TODO: custom errors
    if (_requestId != _dispute.requestId) revert();
    if (_responseId != _dispute.responseId) revert();
  ...
  function _claimPledge(
    ...
    RequestParameters memory _params = decodeRequestData(_requestData);
